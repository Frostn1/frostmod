...
| `--rut-negative-pulse` | — | off | **Private manual test only.** Implies `--rut-diag`, but uses a separate marker so observation alone can never write terrain. After an accepted stock rut stamp, waits for one adjacent cell in a different clean/zero block, writes exactly `-262144` once per process, and marks only that block for the stock transport. It never changes the authoritative/rendered grid or packet payloads directly. MX Bikes beta21e exact-signature gates remain mandatory. See `docs/RUT_DIAGNOSTIC.md`. |
