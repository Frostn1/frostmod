...
The clips live in `src/voice/`: 22.05 kHz 16-bit mono PCM WAV, trimmed, normalised to -1 dBFS
peak, each under a second, about 280 KB in all. CMake embeds them in `mxbcoach.dlo` as
`RCDATA` resources 101-110 (100 + kind). They are played through winmm's `waveOut`: one
device, opened on the first cue spoken, 2 buffers, never waited on. `PlaySound` isn't used
because it plays one sound per process and would cut off other plugins' audio, such as
MXBMRP3's spotter. A new cue that is more important than the one playing cuts it off
(`waveOutReset`); otherwise it isn't spoken, so two never overlap. The rules are in
`src/coachvoice.h`, pinned by `tests/coachvoice_test.cpp`, which also checks the committed
clips.

The clips are generated by `tools/voice/make_clips.py` with [Piper](https://github.com/OHF-Voice/piper1-gpl),
an offline neural text-to-speech engine, and the voice
[`en_US-ljspeech-high`](https://huggingface.co/rhasspy/piper-voices/tree/main/en/en_US/ljspeech/high)
(model repository MIT; trained from scratch on the public-domain
[LJ Speech](https://keithito.com/LJ-Speech-Dataset/) dataset). The voices built on other
datasets were avoided: `libritts_r` is fine-tuned from `lessac`, whose Blizzard 2013 data is
under a research-only licence, and `hfc_male` is CC BY-NC-SA. Credits are in `NOTICE`.
...
| Part | Where | Shows |
|---|---|---|
| Cue | x 0.35-0.65, y 0.285-0.335, size 0.040, centred | the cue that's up, as before (it moved down from y 0.14-0.21) |
| Section | x 0.35-0.65, y 0.338-0.362, size 0.022, centred | `name: tip` of the section the rider is in, cut to about 47 characters |
...
| Map | x 0.01-0.13, y 0.77-0.98 | the centreline, north up, the next 5 cue spots, Coach's ghost and the rider |
...
A missing file or key means on. The one exception is the map: it defaults to off when
