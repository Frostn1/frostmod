...
| `--probe-overjump` | — | off | Adds a hex dump of the 0x21C session settings block (`[overjump.hex] …`) to the one-line `[overjump]` verdict, which is logged either way — every session start says whether the **overjump crash** is on for it. Run testing, host with the option unchecked, and join a server that sets `overjump_crash = 0`; the three lines together say whether the client honours the setting it was given. |
