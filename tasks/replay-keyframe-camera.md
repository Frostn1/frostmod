...
## 7. What shipped, and what the RE got slightly wrong

Built on this in `feature/replay-keyframe-camera`. Before writing the resolver every
signature in §1–§3 was re-run against `mxbikes.exe.unpacked.exe` and every displacement
resolved; the RE holds, with three corrections worth recording:

- **S7 (roll) is not unique** — it matches twice, at `0xC6F2B` (`0x4C91A8`, right) and
  `0xC8170` (`0x4C91F0`, wrong). It is not used. Roll is taken as yaw+8 instead, and the
  pitch signature is resolved purely to check pitch really is yaw+4 on the build in front
  of us before any angle is written.
- **S12's documented displacement offsets are off by one.** The second `movss` in that
  pattern starts at +23 with its disp at +27, not +26, and pos.z is past the end of the
  quoted bytes. Only pos.x is resolved from it; y and z are +4 and +8, which the
  pattern's own second store confirms.
- **S10 and S12 both match twice** (the same store inlined into two branches). Both
  matches resolve to the same global, so the resolver requires every match of a pattern
  to agree rather than taking the first — which is also what would catch a future build
  making one of these ambiguous.

The clock block deltas were confirmed from the game's own transport handlers: rewind-to-
start copies `[cur+0xC]` into `cur`, and the slider reads `[cur+0xC]` and `[cur-0x14]` as
its two ends.

**The hook at `0xC50B1` was not needed.** Playback writes the pose from the existing
per-frame `Tick()` (the `wglSwapBuffers` hook we already own), so the feature installs no
new hook and pins no code address at all. The cost is one frame of latency — the pose
written at present time is the one the *next* frame renders — which is invisible at
replay speed and buys immunity to the offset churn a mid-function hook would have carried.

**The mode is held through `SpectateCameras`, as recommended.** Injected mode has no
callbacks, so it writes `0x4CA2A0` directly each frame instead; both paths only engage
while a path is armed.

### Still needs a Windows session

Everything below is minutes, and none of it blocks the build — the feature ships with each
of these guarded so a wrong answer is visible rather than dangerous.

1. **Sign conventions on screen** for yaw/pitch/roll (§6.1). Interpolation is
   sign-agnostic, so this only shows up as a path that keys correctly and plays back
   mirrored. Set two keys 90° apart and watch which way it sweeps.
2. **Does `SpectateCameras` returning `N+3` actually enter free-roam?** The whole
   playback path depends on it. If it doesn't, the log will show the pose being written
   every frame with nothing on screen changing.
3. **Scrub round-trip** (§6.4): `,` / `.` jump the clock to the previous/next key. If the
   stream does not re-seek, the picture freezes while the clock moves — in which case the
   `0x1400bd590` call the game's own frame-step makes is the missing piece.
4. **Editor key clashes.** `K X C P , . S L` and the digits are pressed while the game's
   replay screen also sees them. Any that the replay screen already uses need moving.
5. **Position units** (§6.6) — only matters for the on-screen readout, not for playback.
6. Replay object index `0xE53DC8` (§6.5) is untouched by this build.
