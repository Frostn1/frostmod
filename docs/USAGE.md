...
| `--rut-diag` | — | off | Phase-0 terrain diagnostic for **MX Bikes beta21e only**. Observes the native rut point writer and received-block apply path, logging caller/thread, position, magnitude, four-cell pre/post signed values, dirty blocks, and received signed-block summaries. It never changes a value or packet. The DLL refuses each hook unless the beta21e timestamp and exact signature match. |
| `--rut-negative-pulse` | — | off | **Private manual test only.** Implies `--rut-diag`, but uses a separate marker so observation alone can never write terrain. After an accepted stock rut stamp, selects one adjacent cell in a different clean/zero block without changing it, then writes exactly `-262144` once at that block's verified pre-compression boundary after the stock sender consumes its queue marker. It never changes the authoritative/rendered grid or compressed packet bytes directly. MX Bikes beta21e exact-signature gates remain mandatory. See `docs/RUT_DIAGNOSTIC.md`. |
| `--probe-mount` | — | off | RE diagnostic only — logs the content-loader's arguments (`[mount] …`). Not needed for normal use. |
| `--probe-overjump` | — | off | RE diagnostic only. Swaps the engine's command-bus fn-ptr so every session start logs whether the **overjump crash** is on for it (`[overjump] …`), plus a hex dump of the 0x21C settings block (`[overjump.hex] …`). That one pointer carries *every* command the game issues, so arm it for a run you are watching and not for normal play — armed by default in v0.16.1, it crashed MX Bikes at every session start. Run testing, host with the option unchecked, and join a server that sets `overjump_crash = 0`; the three lines together say whether the client honours the setting it was given. |
...
| `frostmod_filter.flag`, `frostmod_dumplist.flag`, `frostmod_probe.flag`, `frostmod_rutdiag.flag`, `frostmod_rut_negative_pulse.flag`, `frostmod_unsafe_reload.flag` | Internal on/off markers the launcher writes so the DLL knows which optional hooks to install. You don't edit these; the flags above manage them. |
