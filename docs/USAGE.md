...
| `--rut-negative-pulse` | — | off | **Private manual test only.** Implies `--rut-diag`, but uses a separate marker so observation alone can never write terrain. After an accepted stock rut stamp, selects one adjacent cell in a different clean/zero block without changing it, then writes exactly `-262144` once at that block's verified pre-compression boundary. It never changes the authoritative/rendered grid or compressed packet bytes directly. MX Bikes beta21e exact-signature gates remain mandatory. See `docs/RUT_DIAGNOSTIC.md`. |
